 ...
 * input program
 * @return list of parses: the second item refers to the starting token index,
 * the FlippedEarleyItem's end is the ending token index (exclusive)a, the
 * FlippedEarleyItem's rule is the index in grammar_rules that applies to the
 * partial parse, and the third item is index in the list of the parent of the
 * current parse. The parent of the first item is defaulted to 0 and not
 * informative.
 */
