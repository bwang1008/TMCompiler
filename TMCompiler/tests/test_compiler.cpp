...
TEST_CASE(program_two_functions) {
	...
	Compiler compiler(lex_bnf, syn_bnf);
	const std::string program_text{"void foo() {} int compute(int y) { return 5; }"};
	compiler.compile_text(program_text);
...
TEST_CASE(two_basic_statements) {
	...
	Compiler compiler(lex_bnf, syn_bnf);
	const std::string program_text{"void foo() { int sum = 0; int extra = -25; }"};
	compiler.compile_text(program_text);
...
TEST_CASE(for_loop) {
	...
	Compiler compiler(lex_bnf, syn_bnf);
	const std::string program_text{"int foo() { int sum = 0; for(int i = 0; i < 10; i += 1) { sum += i; } return sum; }"};
	compiler.compile_text(program_text);
...
TEST_CASE(if_statement) {
	...
	Compiler compiler(lex_bnf, syn_bnf);
	const std::string program_text{"int foo() { if(true) { return 1; } return -1; }"};
	compiler.compile_text(program_text);
...
TEST_CASE(if_else_statement) {
	...
	Compiler compiler(lex_bnf, syn_bnf);
	const std::string program_text{"int foo() { if(true) { return 1; } else { return -1; } }"};
	compiler.compile_text(program_text);
...
TEST_CASE(if_else_if_else_statement) {
	...
	Compiler compiler(lex_bnf, syn_bnf);
	const std::string program_text{"bool check() { return true; } int foo() { if(check()) { return 1; } return -1; }"};
	compiler.compile_text(program_text);
